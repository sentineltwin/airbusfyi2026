...
### Option A — Double-click `LAUNCH.bat` (Easiest)

> Locate **`LAUNCH.bat`** in the project root and **double-click** it.  
> Two console windows will open (Backend + Frontend) and your browser will open automatically at **http://localhost:5173**
...
### Option C — Python orchestrator (full Docker stack)

```bash
python launch.py               # auto-detect mode
python launch.py --mode local  # local dev (no Docker required)
python launch.py --stop        # graceful shutdown
```

The launcher automatically:
1. Kills any old processes on ports 8000 / 5173
2. Validates Python 3.9+ and Node.js
3. Installs npm packages on first run (~2 min)
4. Starts the FastAPI backend on **:8000**
5. Starts the Vite frontend on **:5173**
6. Waits for both to be healthy, then opens the browser

### Stop Everything

```
STOP.bat     ← double-click to kill backend + frontend
```
...
*SentinelTwin v4.2.1 — Generated 2026-05-21 — EASA DO-326A Compliant*
